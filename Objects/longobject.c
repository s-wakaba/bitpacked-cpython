...
BITPACKED_LONG_ASSURE_CONVENTIONAL_BEGIN(b)
    if (Py_ABS(Py_SIZE(a)) == 1 && Py_ABS(Py_SIZE(b)) == 1) {
        div = (PyLongObject*)fast_floor_div((PyLongObject*)a, (PyLongObject*)b);
    }
    else if (l_divmod((PyLongObject*)a, (PyLongObject*)b, &div, NULL) < 0) {
        div = NULL;
    }
...
BITPACKED_LONG_ASSURE_CONVENTIONAL_BEGIN(b)
    if (Py_ABS(Py_SIZE(a)) == 1 && Py_ABS(Py_SIZE(b)) == 1) {
        mod = (PyLongObject*)fast_mod((PyLongObject*)a, (PyLongObject*)b);
    }
    else if (l_divmod((PyLongObject*)a, (PyLongObject*)b, NULL, &mod) < 0) {
        mod = NULL;
    }
BITPACKED_LONG_ASSURE_CONVENTIONAL_END(b)
BITPACKED_LONG_ASSURE_CONVENTIONAL_END(a)
